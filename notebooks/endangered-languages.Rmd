...
popup <- paste0("<b>English name:</b>", UN_UNESCO$english_name,"<br>",
...
leaflet() %>% 
  ...
  addCircleMarkers(data = vulnerable, radius = log10(UN_UNESCO$number_of_speakers), opacity = .6, color = 'cornsilk2', group = 'Vulnerable',
                   lng = ~Longitude, 
                   lat = ~Latitude,
                   popup = popup)  %>%
  
  addCircleMarkers(data = definite, radius = log10(UN_UNESCO$number_of_speakers), opacity = .6, color = 'yellow', group = 'Definitely endangered',
                   lng = ~Longitude, 
                   lat = ~Latitude,
                   popup = popup)  %>%
  
  addCircleMarkers(data = critical, radius = log10(UN_UNESCO$number_of_speakers), opacity = .6, color = 'orange', group = 'Critically endangered',
                   lng = ~Longitude, 
                   lat = ~Latitude,
                   popup = popup)  %>%
  
  addCircleMarkers(data = severe, radius = log10(UN_UNESCO$number_of_speakers), opacity = .6, color = 'red', group = 'Severely endangered',
                   lng = ~Longitude, 
                   lat = ~Latitude,
                   popup = popup)  %>%
  
  addCircleMarkers(data = extinct, radius = log10(UN_UNESCO$number_of_speakers), opacity = .6, color = 'black', group = 'Extinct',
                   lng = ~Longitude, 
                   lat = ~Latitude,
                   popup = popup)  %>%
  
  addLayersControl(overlayGroups = c('Vulnerable', 'Definitely endangered', 'Critically endangered', 'Severely endangered', 'Extinct')) %>% 
  addLegend(pal = pal, position = 'bottomright', values = c("Vulnerable",
                                                            "Definitely endangered",
                                                            "Severely endangered", 
                                                            "Critically endangered",
                                                            "Extinct"), title = 'Endangerment degrees' 
  )
```
```{r}
UN_UNESCO %>% 
