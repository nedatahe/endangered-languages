...
make_popups <- function(dataframe){
  ...
  popup <- paste("<b>English name:</b>", dataframe$english_name,"<br>",
                 ...
                 "<b>Family:</b>", dataframe$family,"<br>",
...
```{r}
write.csv(UN_UNESCO,'../data/UN_UNESCO.csv')
write.csv(UN,'../data/UN.csv')
```

```{r}
UN_UNESCO %>% 
  group_by(Country) %>% 
ggplot(aes(y =vitality_level_mean, x = country_GDP_per_capita_US_dollars)) +
  geom_point() 
```
```{r}
UN_UNESCO %>% 
...
```{r}
UN_UNESCO %>% 
  max(number_of_speakers, na.rm = F)
```
